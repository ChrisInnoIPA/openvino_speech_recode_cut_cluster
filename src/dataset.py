...
class BaseLoad:
    def __init__(self, sr, n_mfcc=40):  # n_mfcc 40維梅爾濾波器組
        self.sr = sr
        #print(self.sr, "self.sr====================>\n")
        self.n_mfcc = n_mfcc
        #print(self.n_mfcc, "self.n_mfcc====================>\n")
        self._mfcc = MFCC(sr, n_mfcc=40, log_mels=True)
        #print(self._mfcc, "self._mfcc====================>\n")
        
    def _load(self, path, mfcc=True):
        try:
            waveform, ori_sr = torchaudio.load(path)  #load audio
            waveform = waveform.mean(0, keepdims=True)
        except RuntimeError:
            raise Exception(f"Error loading {path}")
        _resample = Resample(ori_sr, self.sr)  #44100 change to 16000
